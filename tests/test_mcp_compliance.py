...
async def test_tool_schema_validation():
    ...
    for tool in tools:
        ...
        if 'properties' in schema:
            assert isinstance(schema['properties'], dict), f"Tool {tool.name} properties must be dict"
            ...
            for prop_name, prop_def in schema['properties'].items():
                ...
                if 'anyOf' in prop_def:
                    ...
                    assert 'type' in prop_def, f"Tool {tool.name} property {prop_name} missing type"
                
        # Check required fields if present
        if 'required' in schema:
            assert isinstance(schema['required'], list), f"Tool {tool.name} required must be list"
...
async def test_tool_descriptions():
    ...
    for tool in tools:
        ...
        assert not tool.description.startswith("TODO"), f"Tool {tool.name} has placeholder description"
        assert not tool.description.lower().startswith("placeholder"), f"Tool {tool.name} has placeholder description"
            ...
            for param_name, param_def in tool.inputSchema['properties'].items():
                if param_name in expected_types:
                    ...
                    if 'anyOf' in param_def:
                        ...
                        assert set(types) == set(expected), f"Tool {tool.name} param {param_name} type mismatch"
                    else:
                        assert param_type == expected, f"Tool {tool.name} param {param_name} type mismatch"